...
class Crawl(utils.Browser, utils.Aiohttp, utils.Sem):
    def __init__(self, row):
        ...
        # Nota: esto lo que se activaba, deberia tener funcion o lambda
    ...
    async def process(self, url):
        ...
        try:
            ...
            async with self.client.get(url, allow_redirects=True) as resp:
                self.done[url] = True
    ...
    async def parse(self, url, source):
        ...
        results = await utils.gather(*[curl.sem(curl.aioeurl, turl) for turl in urls
